...
class AsrServiceTests(unittest.TestCase):
    def test_rejects_unsupported_empty_and_oversized_audio(self) -> None:
        service = TranscriptionService(AsrSettings(max_audio_bytes=4))
    ...
    def test_cpu_asr_configuration_is_explicitly_unavailable(self) -> None:
        service = TranscriptionService(AsrSettings(device="cpu"))
        with self.assertRaises(TranscriptionError) as unavailable:
            service.transcribe(b"audio", "audio/webm")
        self.assertEqual((unavailable.exception.code, unavailable.exception.status_code), ("gpu_required", 503))

    def test_disabled_service_is_explicitly_unavailable(self) -> None:
        service = TranscriptionService(AsrSettings(ready_override=False))
        with self.assertRaises(TranscriptionError) as unavailable:
            service.transcribe(b"audio", "audio/webm")
        self.assertEqual((unavailable.exception.code, unavailable.exception.status_code), ("model_not_ready", 503))

    def test_faster_whisper_adapter_uses_cuda_transcription_options(self) -> None:
        ...
        result = FasterWhisperAdapter(AsrSettings(), model_factory=fake_factory).transcribe(b"fixture", "audio/wav")
        self.assertEqual(result.text, "我的钥匙在哪里？")
        self.assertEqual(result.provider, "faster-whisper")
        self.assertEqual(calls["model_options"], {"device": "cuda", "compute_type": "float16"})
    ...
    def test_empty_faster_whisper_result_is_not_reported_as_success(self) -> None:
        ...
        adapter = FasterWhisperAdapter(AsrSettings(), model_factory=lambda *args, **kwargs: EmptyModel())
