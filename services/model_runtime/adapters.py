...
def _move(inputs: Any, device: str):
...
class FlorenceAdapter(ModelAdapter):
    def load(self) -> None:
        ...
        self.model, self.torch = Florence2ForConditionalGeneration.from_pretrained(name).to(_device()).eval(), torch

    def infer(self, request: InferenceRequest) -> dict[str, Any]:
        image, task = _image(request), str(request.payload.get("task", "<OD>"))
        if task not in {"<OD>", "<CAPTION>", "<DETAILED_CAPTION>"}: task = "<OD>"
        inputs = _move(self.processor(text=task, images=image, return_tensors="pt"), _device())
...
class GroundingAdapter(ModelAdapter):
    def load(self) -> None:
        ...
        self.processor = AutoProcessor.from_pretrained(name)
        self.model, self.torch = AutoModelForZeroShotObjectDetection.from_pretrained(name).to(_device()).eval(), torch
    ...
    def infer(self, request: InferenceRequest) -> dict[str, Any]:
        ...
        inputs = _move(self.processor(images=image, text=". ".join(candidates) + ".", return_tensors="pt"), _device())
...
class SamAdapter(ModelAdapter):
    def load(self) -> None:
        ...
        self.model, self.torch = Sam2Model.from_pretrained(name).to(_device()).eval(), torch
    ...
    def infer(self, request: InferenceRequest) -> dict[str, Any]:
        ...
        inputs = _move(self.processor(images=image, input_boxes=[boxes], return_tensors="pt"), _device())
...
class EmbeddingAdapter(ModelAdapter):
    def load(self) -> None:
        ...
        self.model, self.torch = AutoModel.from_pretrained(name).to(_device()).eval(), torch
    ...
    def infer(self, request: InferenceRequest) -> dict[str, Any]:
        ...
        for item in detections:
            ...
            inputs = _move(self.processor(images=crop, return_tensors="pt"), _device())
