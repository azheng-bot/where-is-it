...
class PipelineTests(unittest.TestCase):
    ...
    def test_real_pipeline_only_publishes_verified_segment(self):
        with patch.object(main, "VISION_MODE", "real"), patch.object(main.video_source, "read_jpeg", return_value="ZmFrZQ=="), patch.object(main.InternalServiceClient, "infer", self.success):
    ...
    def test_timeout_stops_unverified_observation(self):
        with patch.object(main, "VISION_MODE", "real"), patch.object(main.video_source, "read_jpeg", return_value="ZmFrZQ=="), patch.object(main.InternalServiceClient, "infer", side_effect=ServiceCallError(ErrorCode.DEADLINE_EXCEEDED, "timeout")):
    ...
    def test_contract_mismatch_stops_unverified_observation(self):
        with patch.object(main, "VISION_MODE", "real"), patch.object(main.video_source, "read_jpeg", return_value="ZmFrZQ=="), patch.object(main.InternalServiceClient, "infer", side_effect=ServiceCallError(ErrorCode.CONTRACT_MISMATCH, "bad version", False)):
            self.assertIsNone(main.run_pipeline())
